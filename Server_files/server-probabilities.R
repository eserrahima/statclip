...
source('Server_files/helper-probabilities.R/server-probabilities_chiTail.R', local=TRUE)
source('Server_files/helper-probabilities.R/server-probabilities_FTail.R', local=TRUE)
source('Server_files/helper-probabilities.R/server-probabilities_normTail.R', local=TRUE)
...
probabilities_tail <- renderUI(
{
  #print("tail")
  if (input$prob_dist == "rbinom")
  {
  ...
  else if (input$prob_dist == "rf" | input$prob_dist == "rchisq"){
    selectInput(inputId = "tail",
                label = "Find Area:",
...
probabilities_lower_bound <- renderUI(
{
  #print("lower bound")
...
probabilities_upper_bound <- renderUI(
{
  #print("upper bound")
...
get_model_text = reactive(
{
  ...
  if (input$prob_dist == "rbinom" & input$tail != "equal")
  {
...
probabilities_model <- renderText(
{
  #print("model")
...
probabilities_mean <- renderUI(
{
  #print("mean")
  if (input$prob_dist == "rnorm")
  {
    sliderInput("mu",
                "Mean",
                value = 0,
...
probabilities_df1 <- renderUI(
{
  #print("df1")
  if (input$prob_dist %in% c("rt","rchisq","rf"))
  {
    sliderInput(ifelse(input$prob_dist %in% c("rt","rchisq"), "df","df1"),
                "Degrees of freedom",
                value = 10,
                min = 1,
                max = 50)
...
probabilities_df2 <- renderUI(
{
  #print("df2")
  if (input$prob_dist == "rf")
  {
    sliderInput("df2",
...
probabilities_a <- renderUI(
{
  ...
  else if (input$prob_dist == "rchisq")
  {
  ...
  else if (input$prob_dist == "rbinom")
  {
    if (is.null(input$n)){
      shiny:::flushReact()
      return()
...
{ 
  ...
  {
    if (input$prob_dist == "rnorm" | input$prob_dist == "rt") 
    {
      M = NULL
      if (input$tail == "middle")
      {
